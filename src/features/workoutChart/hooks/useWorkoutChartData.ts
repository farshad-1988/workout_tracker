...
//   const model = useMemo(() => {
//     const today = makeDO(undefined, locale);
//     const todayKey = today.format("YYYY-MM-DD");

//     if (period === "week") {
//       const base = getStartOfWeek(today);
//       const shifted = makeDO(base.toDate(), locale);
//       shifted.add(offset * 7, "day");
//       const dateKeys = getPersianWeekDateKeys(shifted, locale);
//       const end = cloneDO(shifted, locale);
//       end.add(6, "day");
...
//     if (period === "month") {
//       const anchor = makeDO(today.toDate(), locale);
//       anchor.add(offset, "month");
//       const { dateKeys, dayLabels } = getPersianMonthSeries(anchor, locale);
//       const stats = computeCaloriesSeries(state, dateKeys, (key) => {
//         return offset === 0 && isPersianDateKeyAfterToday(key, todayKey);
//       });
//       return {
...
//         dateKeys,
//         periodLabel: makePersianMonthRangeLabel(anchor, locale),
//         stats,
//         chartData: makeChartData(stats.values, dayLabels),
//       };
//     }

//     const anchorYear = makeDO(today.toDate(), locale);
...
export function useWorkoutChartData(
  ...
  const model = useMemo(() => {
    ...
    if (period === "week") {
      const base = getStartOfWeek(today, locale);
      const shifted = makeDO(base.toDate(), locale);
      shifted.add(offset * 7, "day");
      const dateKeys = getWeekDateKeys(shifted, locale);
      const end = cloneDO(shifted, locale);
      end.add(6, "day");
      const periodLabel = `${shifted.format("YYYY/MM/DD")} – ${end.format("YYYY/MM/DD")}`;
      const stats = computeCaloriesSeries(
        state,
        dateKeys,
        (key) => offset === 0 && isDateKeyAfterToday(key, todayKey),
      );
      return {
        pointLabels: weekDayLabels,
        dateKeys,
    ...
    if (period === "month") {
      const anchor = makeDO(today.toDate(), locale);
      anchor.add(offset, "month");
      const { dateKeys, dayLabels } = getMonthSeries(anchor, locale);
      const stats = computeCaloriesSeries(
        state,
        dateKeys,
        (key) => offset === 0 && isDateKeyAfterToday(key, todayKey),
      ...
      return {
        pointLabels: dayLabels,
        dateKeys,
        periodLabel: makeMonthRangeLabel(anchor, locale),
        stats,
        chartData: makeChartData(stats.values, dayLabels),
      };
    }
    ...
    const anchorYear = makeDO(today.toDate(), locale);
    anchorYear.add(offset, "year");
    const year = Number(anchorYear.year) || today.year;
    const { labels, values } = getYearMonthSeries(year, state, today, locale);
    ...
    return {
      pointLabels: labels,
      dateKeys: labels,
      periodLabel: makeYearLabel(year),
      stats: {
        values,
        totalCalories,
        avgCalories: counted > 0 ? Math.round(totalCalories / counted) : 0,
  ...
  }, [offset, period, state, weekDayLabels, locale]);
